...
**作者：DeepSeek-Whale-Desktop-Pet 团队**　|　版本：`v4.1.0`　|　[开发者信息](#-开发者信息)
...
- 🐟 **互动玩法**：鼠标跟随、躲避鼠标、待机漫游（走走停停 + 边缘弱回正）
- 🎨 **表情系统**：内置 v1~v4 表情包 + 默认表情 + Gif 摸头，
  连点 N 次换表情（1~100 可调）、自动轮换（5~100 秒可调）、空闲眨眼动画
- ⚙️ **系统能力**：单实例锁、独立运行日志（自动轮转）、应用打开监控 +
  自定义规则、自定义点击音效、GitHub 更新检查、托盘右键菜单
...
开机自启 · **应用打开监控**（匹配进程名/窗口标题 → 自定义台词，可增删/启停规则） ·
检查更新 · 自定义音效（设置/清除） · 打开运行日志
...
├── assets/
│   └── DSH2.png            # README 封面图
└── expressions/            # 表情与音效素材（v1~v4 + 默认表情 + Gif + mp3）
```

## 📄 配置与日志

- 配置文件：`C:\Users\<用户名>\.dshw-desktop-pet.json`（自动保存）
- 运行日志：`%TEMP%\dshw-pet.log`（超过 512KB 自动轮转为 `.old`）
