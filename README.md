...
**作者：shiyi312**　|　版本：`v4.4.0`　|　[开发者信息](#-开发者信息)
...
项目内置两条互补的检查路径，**双击根目录 `自测.bat`** 即可全部跑一遍，
报告输出到 `tests\自测报告.txt`：

| 路径 | 作用 |
|---|---|
| **静态分析** `tests/check_static.py` | 语法编译 + pyflakes（未定义变量 / 重复定义 / 未使用）+ AST 扫描（注释里被吞掉的赋值语句、函数内未定义变量、超长函数提示） |
